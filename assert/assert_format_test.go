...
func TestConsistentlyf(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		mock := new(mockT)
		result := Consistentlyf(mock, func() bool { return true }, 100*time.Millisecond, 20*time.Millisecond, "test message")
		if !result {
			t.Error("Consistentlyf should return true on success")
		}
	})
	...
	t.Run("failure", func(t *testing.T) {
		...
		mock := new(mockT)
		result := Consistentlyf(mock, func() bool { return false }, 100*time.Millisecond, 20*time.Millisecond, "test message")
		if result {
			t.Error("Consistentlyf should return false on failure")
		}
		if !mock.failed {
			t.Error("Consistentlyf should mark test as failed")
		}
	})
}
...
func TestErrorNotContainsf(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		mock := new(mockT)
		result := ErrorNotContainsf(mock, ErrTest, "not in message", "test message")
		if !result {
			t.Error("ErrorNotContainsf should return true on success")
		}
	})
	...
	t.Run("failure", func(t *testing.T) {
		...
		mock := new(mockT)
		result := ErrorNotContainsf(mock, ErrTest, "general error", "test message")
		if result {
			t.Error("ErrorNotContainsf should return false on failure")
		}
		if !mock.failed {
			t.Error("ErrorNotContainsf should mark test as failed")
		}
	})
}
...
func TestEventuallyWithf(t *testing.T) {
	...
	t.Run("failure", func(t *testing.T) {
		...
		mock := new(mockT)
		result := EventuallyWithf(mock, func(c *CollectT) { c.Cancel() }, 100*time.Millisecond, 20*time.Millisecond, "test message")
		if result {
			t.Error("EventuallyWithf should return false on failure")
		}
		if !mock.failed {
